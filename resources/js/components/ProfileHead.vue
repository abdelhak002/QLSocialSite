        ...
        <div class="mt-3">
          ...
          <div class="text-gray-600 flex">
            ...
            <span class="flex mr-2">
              ...
              <span class="leading-5 ml-1">Joined {{ profile.for_page_created_at }}</span>
            </span>
          </div>
        </div>
        <div
...
export default {
    ...
    mounted: function(){
        this.profile = JSON.parse(document.getElementById('profilejson').innerHTML);
    },
    methods:{
        unfollow: function(){
            axios.post(`/u/${this.profile.id}/unfollow`)
            .then(res => {
        ...
        follow: function(){
            axios.post(`/u/${this.profile.id}/follow`)
            .then(res => {
